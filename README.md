...
1. Clone the DengueChat repository from gitkraken (https://github.com/socialappslab/denguetorpedo). Step inside the folder where the code is cloned 
...
3. Clone this repository and step inside the folder
 ...
 clone https://github.com/socialappslab/denguechat-compose.git



If you want to run this on a local machine, for development purposes, be sure to first check the `localdev` branch, you can use gitkraken
...
```yaml
  volumes:
    - E:\Usuario\Desktop\TrabajoSE2\denguetorpedo:/home/dengue/denguetorpedo
```
...
8. In the new `.env` file, edit the variable `DATABASE_URL` to include the right database user, password and name: 
	...
	DATABASE_URL=protocol://username:password@host:port/databasename


9. In the new `.env` file, edit `REDIS_PASSWORD` with the right pass.  

12 Build and run REDIS:

 E:\Usuario\Desktop\TrabajoSE2\denguechat-composedocker-compose build redis 


 E:\Usuario\Desktop\TrabajoSE2\denguechat-composedocker-compose up -d redis
 
13.On the file denguetorpedo on the Dockerfile add the followin if not there RUN printf "deb http://archive.debian.org/debian/ jessie main\ndeb-src http://archive.debian.org/debian/ jessie main\ndeb http://security.debian.org jessie/updates main\ndeb-src http://security.debian.org jessie/updates main" > /etc/apt/sources.list

14.Build and run denguetorpedo
 
 E:\Usuario\Desktop\TrabajoSE2\denguetorpedo>docker-compose build denguetorpedo 
  
 E:\Usuario\Desktop\TrabajoSE2\denguetorpedo>docker-compose up -d denguetorpedo 
 
15.Run the following docker-compose logs -f --tail=100 denguetorpedo
...
Access DengueChat UI at http://localhost:5000
